...
% get the magnetic flux density in the core (normalized per current unit)
B_tot = I_winding.*L_norm./A_core;
B_norm = B_tot./I_winding;

% get the current density in the winding (normalized per current unit)
J_tot = I_winding./A_winding;
J_norm = J_tot./I_winding;

% get the magnetic field in the winding (normalized per current unit)
