classdef CoreData < handle
    ...
    methods (Access = public)
        ...
        function [is_valid, P] = get_losses_sin(self, f, B_peak, B_dc, T)
            [is_valid, P] = self.get_losses(f, B_peak, B_dc, T);
        ...
        function [is_valid, P] = get_losses_tri(self, f, d_c, B_peak, B_dc, T)
            ...
            [is_valid_tmp, P_ref] = self.get_losses(f, B_peak, B_dc, T);
            is_valid = is_valid&is_valid_tmp;
            
            [is_valid_tmp, P_f_1] = self.get_losses(f_1, B_peak, B_dc, T);
            is_valid = is_valid&is_valid_tmp;

            [is_valid_tmp, P_f_2] = self.get_losses(f_2, B_peak, B_dc, T);
            is_valid = is_valid&is_valid_tmp;

            [is_valid_tmp, P_B_peak_1] = self.get_losses(f, B_peak_1, B_dc, T);
            is_valid = is_valid&is_valid_tmp;

            [is_valid_tmp, P_B_peak_2] = self.get_losses(f, B_peak_2, B_dc, T);
            is_valid = is_valid&is_valid_tmp;
        ...
        function [is_valid, P] = get_losses(self, f, B_peak, B_dc, T)
