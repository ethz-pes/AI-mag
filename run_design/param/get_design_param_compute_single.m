function [eval_ann, data_compute] = get_design_param_compute_single(eval_type)
...
for i=1:length(f_vec)
    ...
    excitation_tmp = get_design_excitation(f_vec(i), L, load_full_load);
    excitation.(tag) = excitation_tmp;
    
    tag = sprintf('partial_load_f_%.0fk', 1e-3.*f_vec(i));
    excitation_tmp = get_design_excitation(f_vec(i), L, load_partial_load);
    excitation.(tag) = excitation_tmp;
end
