classdef AnnFem < handle
    ...
    methods (Access = public)
        function self = AnnFem(data_ann_fem, geom_type, eval_type)
            % Constructor.
            %
            %    Set the data.
            ...
            %
            %    Parameters:
            %        data_ann_fem (struct): contain the results of the magnetic and thermal ANN/regression
            %        geom_type (str): type of the geometry variables ('abs' or 'rel')
            %        eval_type (str): get the result of the ANN/regression, FEM or, just the analytical solution ('ann', 'fem', or 'approx')
            ...
            self.const = data_ann_fem.const;
            
            % create the regression engine and load the data
            [self.file_model_mf, self.ann_manager_mf_obj] = self.get_ann_manager(data_ann_fem.ann_mf, 'mf');
            [self.file_model_ht, self.ann_manager_ht_obj] = self.get_ann_manager(data_ann_fem.ann_ht, 'ht');
    ...
    methods (Access = private)
        ...
        function [is_valid, inp] = get_extend_inp_wrapper(self, model_type, inp)
            ...
            [is_valid, inp] = dataset.get_extend_inp(self.const, model_type, var_type, self.n_sol, inp);
        end
            ...
            switch self.eval_type
                case 'fem'
                    ...
                    for i=1:length(self.n_sol)
                        ...
                        fom(i) = dataset.get_out_fem(model, model_type, inp_tmp);
                    end
                    fom = get_struct_assemble(fom);
                case {'ann', 'approx'}
                    % analytical approximation supports vector evaluation
                    fom = dataset.get_out_approx(model_type, inp);
                otherwise
