...
if any(strcmp(model_type, {'ht', 'mf'}))
    ...
    sweep.var.fact_gap = struct('var_trf', 'log', 'type', 'float', 'lb', 0.005,  'ub', 0.3, 'n', n);
    sweep.var.V_box = struct('var_trf', 'log', 'type', 'float', 'lb', 10e-6,  'ub', 1000e-6, 'n', n);
end
if strcmp(model_type, 'mf')
    sweep.var.J_winding = struct('var_trf', 'log', 'type', 'float', 'lb', 0.001e6,  'ub', 20e6, 'n', n);
end
if strcmp(model_type, 'ht')
    sweep.var.p_density_tot = struct('var_trf', 'log', 'type', 'float', 'lb', 0.001e4,  'ub', 1e4, 'n', n);
    sweep.var.p_ratio_winding_core = struct('var_trf', 'log', 'type', 'float', 'lb', 0.02,  'ub', 50.0, 'n', n);
end
